    ...
    <div className="bg-white pt-3 pb-5 px-5 w-full">
      ...
      <div className="space-y-2">
        {collectives.map((collective) => (
          <div
            key={collective.field}
            className={`p-3 border ${membership && membership[collective.field] === 1 ? 'bg-black text-white' : 'border-gray-300 shadow-sm'} rounded-lg`}
          >
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center">
                <p className={`text-sm ${membership && membership[collective.field] === 1 ? 'hidden text-white' : ''} mr-1`}>Join</p>
                <Toggle
                  defaultChecked={membership && membership[collective.field] === 1}
                  icons={false}
                  onChange={() => handleToggle(collective.field)}
                />
              </div>
            </div>
            <p className={`text-sm mt-2 ${membership && membership[collective.field] === 1 ? 'hidden' : ''}`}>{collective.description}</p>
            {membership && membership[collective.field] === 1 && (
