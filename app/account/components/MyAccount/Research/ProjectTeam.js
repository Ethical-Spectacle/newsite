...
import { FaGlobe, FaGithub, FaLinkedin, FaUsers, FaTrashAlt } from 'react-icons/fa';
...
const ProjectTeam = ({ projectId }) => {
  ...
  const updateStatus = async (email, status) => {
    try {
      const response = await fetch(`${API_URL_PROD}/update_researcher_application_status`, {
        ...
        body: JSON.stringify({ project_id: projectId, email, status }),
      });

      if (response.ok) {
        setAcceptedApplicants(acceptedApplicants.filter(applicant => applicant.email !== email));
      } else {
  ...
  const handleRoleChange = (email, role) => {
    setAcceptedApplicants(acceptedApplicants.map(applicant =>
      applicant.email === email ? { ...applicant, role } : applicant
    ));
  };

  const updateRole = async (email, role) => {
    try {
      const response = await fetch(`${API_URL_PROD}/update_researcher_application_role`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleRoleKeyPress = (e, email, role) => {
    if (e.key === 'Enter') {
  ...
  return (
    <div className="min-h-36 pb-2 md:pb-5 border-b-2 border-gray-300">
      ...
      <div className="w-full">
        {acceptedApplicants.length > 0 ? acceptedApplicants.map((applicant) => (
          <div key={applicant.application_id} className="bg-white py-3 px-4 border border-gray-400 border-1 mb-4 rounded-lg">
            <div className="flex justify-between">
              ...
              <div className="flex space-x-2 justify-end my-auto">
                ...
                <button
                  ...
                  onClick={() => updateStatus(applicant.email, 'declined')}
                >
                  <FaTrashAlt />
                </button>
              </div>
            ...
            <input
              ...
              onChange={(e) => handleRoleChange(applicant.email, e.target.value)}
              onBlur={(e) => updateRole(applicant.email, e.target.value)}
              onKeyPress={(e) => handleRoleKeyPress(e, applicant.email, e.target.value)}
